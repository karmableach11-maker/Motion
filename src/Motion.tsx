...
const RANSOM_EVENTS: RansomEvent[] = [
  {from: 1, to: 4, start: 142, family: "LOCKBIT", id: "RX-041"},
  {from: 4, to: 7, start: 222, family: "BLACKCAT", id: "RX-117"},
  {from: 5, to: 9, start: 302, family: "AKIRA", id: "RX-203"},
  {from: 7, to: 9, start: 382, family: "PLAY", id: "RX-288"},
  {from: 9, to: 10, start: 462, family: "MEDUSA", id: "RX-354"},
  {from: 10, to: 0, start: 542, family: "RHYSIDA", id: "RX-419"},
];

const rcFade = (frame: number) => {
  const intro = interpolate(frame, [1, 76], [0, 1], {
    ...clamp,
    easing: Easing.out(Easing.cubic),
  });
  const outro = interpolate(frame, [824, 899], [1, 0], {
    ...clamp,
    easing: Easing.inOut(Easing.cubic),
  });
  return intro * outro;
...
const RcCorners: React.FC<{
  ...
  color?: string;
  opacity?: number;
}> = ({x, y, width, height, color = RC.line2, opacity = 1}) => {
  const d = 18;
  return (
    <path
      d={`M${x} ${y + d}V${y}H${x + d} M${x + width - d} ${y}H${x + width}V${
        y + d
      } M${x} ${y + height - d}V${y + height}H${x + d} M${x + width - d} ${
        y + height
      }H${x + width}V${y + height - d}`}
      fill="none"
      stroke={color}
      strokeWidth="2"
      opacity={opacity}
...
const ArcMeter: React.FC<{
  ...
  color: string;
  reverse?: boolean;
}> = ({cx, cy, radius, frame, phase, color, reverse = false}) => {
  const spin = (frame * (reverse ? -0.075 : 0.055) + phase) % 360;
  ...
  return (
    <g transform={`rotate(${spin} ${cx} ${cy})`}>
      <circle
        ...
        stroke={color}
        strokeWidth="1.4"
        strokeDasharray={`${circumference * 0.11} ${circumference * 0.065}`}
        strokeOpacity="0.42"
      />
      <circle
        cx={cx + radius}
        ...
        r="3.2"
        fill={color}
        opacity={0.6 + 0.35 * Math.sin(frame * 0.08 + phase)}
      />
    </g>
  );
};

const ContainmentMap: React.FC<{frame: number}> = ({frame}) => {
  const mapReveal = ease(frame, 46, 72);
  const mapPulse = 0.5 + 0.5 * Math.sin(frame * 0.035);
  return (
    <g transform="translate(282 176) scale(1.55)">
      <path
        d={WORLD_LAND_PATH}
        fill="url(#rc-land)"
        fillRule="evenodd"
        stroke={RC.teal}
        strokeWidth="0.7"
        strokeOpacity={0.44 + mapPulse * 0.14}
        opacity={mapReveal}
        style={{transformOrigin: "440px 240px", transform: `scale(${0.96 + mapReveal * 0.04})`}}
      />
      <path
        d={WORLD_BORDER_PATH}
        fill="none"
        stroke={RC.cyan}
        strokeWidth="0.34"
        strokeOpacity={0.18 + mapPulse * 0.08}
        strokeDasharray="1600"
        strokeDashoffset={1600 * (1 - mapReveal)}
      />
      {RANSOM_EVENTS.map((event, index) => {
        const from = GEO_NODES[event.from];
        const to = GEO_NODES[event.to];
        const grow = ease(frame, event.start, 58);
        const contain = ease(frame, event.start + 65, 44);
        const life = frame - event.start;
        const curve = 42 + Math.abs(to.x - from.x) * 0.09;
        const mx = (from.x + to.x) / 2;
        const my = (from.y + to.y) / 2 - curve;
        const route = `M${from.x} ${from.y}Q${mx} ${my} ${to.x} ${to.y}`;
        const localBeat = 0.5 + 0.5 * Math.sin(frame * 0.12 + index * 0.8);
        const heat = grow * (1 - contain * 0.44);
        const ripplePhase = ((Math.max(0, life) % 72) / 72);
        return (
          <g key={event.id} opacity={grow}>
            <path
              d={route}
              fill="none"
              stroke={RC.hot}
              strokeWidth={1.15 + (1 - contain) * 0.85}
              strokeOpacity={0.33 + (1 - contain) * 0.52}
              strokeDasharray="620"
              strokeDashoffset={620 * (1 - grow)}
              strokeLinecap="round"
            />
            <path
              d={route}
              fill="none"
              stroke={contain > 0.7 ? RC.teal : RC.orange}
              strokeWidth="0.52"
              strokeOpacity={0.58}
              strokeDasharray="3 6"
            />
            {[0, 1, 2].map((packet) => {
              const packetProgress =
                ((Math.max(0, life) * (0.0085 + packet * 0.0008) + packet * 0.31) % 1) * grow;
              const pt = quadraticPoint(from, to, curve, packetProgress);
              return (
                <g key={packet} opacity={1 - contain * 0.42}>
                  <circle
                    cx={pt.x}
                    cy={pt.y}
                    r={packet === 0 ? 2.8 : 1.65}
                    fill={packet === 0 ? RC.ink : RC.red}
                    filter="url(#rc-small-glow)"
                  />
                  {packet === 0 ? (
                    <path
                      d={`M${pt.x - 5} ${pt.y}H${pt.x - 1.5}`}
                      stroke={RC.red}
                      strokeWidth="1.1"
                      strokeLinecap="round"
                    />
            ...
            <circle
              ...
              r={4 + heat * 7}
              fill={RC.hot}
              fillOpacity={0.24 + heat * 0.34}
              filter="url(#rc-heat-glow)"
            />
            <circle
              cx={to.x}
              cy={to.y}
              r={9 + ripplePhase * 22}
              fill="none"
              stroke={contain > 0.38 ? RC.teal : RC.red}
              strokeWidth={1.3 - ripplePhase * 0.65}
              strokeOpacity={(1 - ripplePhase) * 0.66}
            />
            <circle
              cx={to.x}
              cy={to.y}
              ...
              stroke={RC.red}
              strokeWidth="0.55"
              strokeOpacity={(1 - ripplePhase) * 0.24 * (1 - contain)}
              strokeDasharray="2 4"
            />
            <g
              transform={`translate(${to.x} ${to.y}) rotate(${frame * 0.18 + index * 29})`}
              opacity={contain}
            >
              <circle
                r={13 + localBeat * 1.4}
            ...
            <circle
              ...
              cy={to.y}
              r={2.3 + localBeat * 0.8}
              fill={contain > 0.55 ? RC.teal : RC.ink}
            />
          </g>
        );
      })}
      {GEO_NODES.map((node, index) => {
        const pulse = 0.5 + 0.5 * Math.sin(frame * 0.055 + index * 1.13);
        return (
          <g key={`ambient-${node.name}`} opacity={0.36 + mapReveal * 0.42}>
            <circle
              cx={node.x}
              cy={node.y}
              r={2.4 + pulse * 0.7}
              fill={index % 3 === 0 ? RC.amber : RC.cyan}
            />
            <circle
              cx={node.x}
              cy={node.y}
              r={5.8 + pulse * 2.2}
              fill="none"
              stroke={index % 3 === 0 ? RC.amber : RC.cyan}
              strokeWidth="0.45"
              strokeOpacity={0.32 + pulse * 0.25}
            />
          </g>
        );
      })}
...
const ThreatRail: React.FC<{frame: number; reveal: number}> = ({frame, reveal}) => {
  const completed = RANSOM_EVENTS.filter((event) => frame >= event.start + 109).length;
  const latest = Math.max(
    0,
    RANSOM_EVENTS.findIndex(
      (event, index) =>
        frame >= event.start &&
        (index === RANSOM_EVENTS.length - 1 || frame < RANSOM_EVENTS[index + 1].start),
    ),
  );
  return (
    <g transform={`translate(${66 - (1 - reveal) * 28} 218)`} opacity={reveal}>
      <text x="0" y="-38" fill={RC.dim} fontSize="9" letterSpacing="2.6">
        THREAT CASCADE
      </text>
      <text x="0" y="-10" fill={RC.ink} fontSize="15" fontWeight="700" letterSpacing="1.8">
        LIVE FAMILIES
      </text>
      {RANSOM_EVENTS.map((event, index) => {
        const active =
          frame >= event.start &&
          frame < (RANSOM_EVENTS[index + 1]?.start ?? 690);
        const isolated = frame >= event.start + 109;
        const pulse = 0.5 + 0.5 * Math.sin(frame * 0.09 + index);
        return (
          <g key={event.id} transform={`translate(0 ${index * 93 + 28})`}>
            <rect
              x="0"
              y="0"
              width="264"
              height="73"
              rx="13"
              fill={active ? RC.hot : RC.glass}
              fillOpacity={active ? 0.09 + pulse * 0.025 : 0.72}
              stroke={isolated ? RC.teal : active ? RC.red : RC.line}
              strokeWidth={active || isolated ? 1.5 : 1}
            />
            <circle
              cx="25"
              cy="25"
              r={active ? 6 + pulse * 1.8 : 5}
              fill={isolated ? RC.teal : active ? RC.red : RC.dim}
            />
            {active ? (
              <circle
                cx="25"
                ...
                fill="none"
                stroke={RC.red}
                strokeOpacity={0.38 - pulse * 0.18}
              />
            ) : null}
            <text x="48" y="25" fill={active ? RC.ink : RC.soft} fontSize="11" fontWeight="700">
              {event.family}
            </text>
            <text x="48" y="45" fill={RC.dim} fontSize="8" letterSpacing="1.3">
              {event.id} / ENCRYPTOR
            </text>
            <text
              x="244"
              ...
              letterSpacing="1.1"
            >
              {isolated ? "ISOLATED" : active ? "PROPAGATING" : "QUEUED"}
            </text>
            <rect x="48" y="56" width="196" height="3" rx="1.5" fill={RC.line} />
            <rect
              x="48"
              y="56"
              width={196 * ease(frame, event.start, 72)}
              height="3"
              rx="1.5"
              fill={isolated ? RC.teal : active ? RC.red : RC.amber}
              opacity="0.88"
            />
          </g>
        );
      })}
      <text x="0" y="602" fill={RC.dim} fontSize="8" letterSpacing="1.3">
        CONTAINED {String(completed).padStart(2, "0")} / ACTIVE{" "}
        {frame < 142 || frame > 690 ? "00" : "01"}
      </text>
      <path
        d={`M0 620H${264 * ease(frame, 88, 90)}`}
        stroke={latest > 3 ? RC.red : RC.amber}
        strokeWidth="2"
      />
    </g>
  );
...
const ResponseRail: React.FC<{frame: number; reveal: number}> = ({frame, reveal}) => {
  const completed = RANSOM_EVENTS.filter((event) => frame >= event.start + 109).length;
  const response = 31 - completed * 3 + Math.round(Math.sin(frame * 0.023) * 2);
  const peak = frame >= 560 && frame < 690;
  return (
    <g transform={`translate(${1590 + (1 - reveal) * 28} 218)`} opacity={reveal}>
      <text x="0" y="-38" fill={RC.dim} fontSize="9" letterSpacing="2.6">
        CONTAINMENT CORE
      </text>
      <text x="0" y="-10" fill={RC.ink} fontSize="15" fontWeight="700" letterSpacing="1.8">
      ...
      <g transform="translate(132 102)">
        {[0, 1, 2].map((ring) => (
          <ArcMeter
            ...
            frame={frame}
            phase={ring * 74}
            color={ring === 2 && peak ? RC.red : ring === 1 ? RC.amber : RC.teal}
            reverse={ring === 1}
          />
        ))}
        <circle r="37" fill={RC.glass2} stroke={peak ? RC.red : RC.line2} />
        <text y="-2" textAnchor="middle" fill={RC.ink} fontSize="29" fontWeight="700">
          {String(84 + completed * 2).padStart(2, "0")}
        </text>
        <text y="18" textAnchor="middle" fill={RC.dim} fontSize="7" letterSpacing="1.2">
          AUTO RESPONSE
        </text>
      </g>
      {([
        ["ISOLATION", completed / 6, RC.teal],
        ["KEY VAULT", 0.91 + Math.sin(frame * 0.015) * 0.018, RC.cyan],
        ["BACKUP LOCK", 0.83 + completed * 0.018, RC.amber],
        ["EDR COVERAGE", 0.94 + Math.sin(frame * 0.012) * 0.014, RC.teal],
      ] as const).map((row, index) => {
        const breathe = 0.5 + 0.5 * Math.sin(frame * 0.045 + index);
        return (
          <g key={row[0]} transform={`translate(0 ${245 + index * 62})`}>
            <text x="0" y="0" fill={RC.soft} fontSize="9" letterSpacing="1.1">
              {row[0]}
            </text>
            <text x="264" y="0" textAnchor="end" fill={row[2]} fontSize="9" fontWeight="700">
              {Math.round(Math.min(0.99, Number(row[1])) * 100)}%
            </text>
            <rect x="0" y="16" width="264" height="7" rx="3.5" fill={RC.glass2} stroke={RC.line} />
            <rect
              x="2"
              y="18"
              width={260 * Math.min(0.99, Number(row[1]))}
              height="3"
              rx="1.5"
              fill={row[2]}
              opacity={0.66 + breathe * 0.32}
            />
          </g>
        );
      })}
      <g transform="translate(0 525)">
        <rect width="264" height="94" rx="14" fill={RC.glass} stroke={RC.line} />
        <text x="17" y="26" fill={RC.dim} fontSize="8" letterSpacing="1.2">
          MEAN ISOLATION TIME
        </text>
        <text x="17" y="61" fill={RC.ink} fontSize="27" fontWeight="700">
          00:{String(response).padStart(2, "0")}
        </text>
        <text x="245" y="61" textAnchor="end" fill={RC.teal} fontSize="9" fontWeight="700">
          SEC
        </text>
        <circle cx="238" cy="23" r={4 + 1.5 * (0.5 + 0.5 * Math.sin(frame * 0.08))} fill={RC.teal} />
      </g>
    </g>
  );
};

const EventHorizon: React.FC<{frame: number; reveal: number}> = ({frame, reveal}) => {
  const bars = Array.from({length: 72}, (_, index) => {
    const eventEnergy = RANSOM_EVENTS.reduce((sum, event, eventIndex) => {
      const distance = Math.abs(frame - event.start - index * 1.1);
      return sum + Math.max(0, 1 - distance / (82 + eventIndex * 5));
    }, 0);
    return Math.min(
      1,
      0.15 +
        seed(index * 9 + 61) * 0.25 +
        (0.5 + 0.5 * Math.sin(frame * 0.04 + index * 0.39)) * 0.15 +
        eventEnergy * 0.34,
    );
  });
  return (
    <g transform={`translate(364 ${913 + (1 - reveal) * 22})`} opacity={reveal}>
      <rect width="1192" height="99" rx="18" fill={RC.glass} fillOpacity="0.78" stroke={RC.line} />
      <text x="22" y="27" fill={RC.dim} fontSize="8" letterSpacing="1.6">
        ENCRYPTION ACTIVITY / LIVE EVENT HORIZON
      </text>
      <text x="1170" y="27" textAnchor="end" fill={RC.red} fontSize="8" fontWeight="700">
        SEVERITY HIGH
      </text>
      <line x1="22" y1="78" x2="1170" y2="78" stroke={RC.line2} strokeOpacity="0.54" />
      {bars.map((value, index) => {
        const hot = value > 0.72;
        return (
          <rect
            key={index}
            x={24 + index * 15.9}
            ...
            height={value * 39}
            rx="2.2"
            fill={hot ? RC.red : index % 5 === 0 ? RC.amber : RC.teal}
            opacity={0.5 + value * 0.5}
          />
        );
      })}
      {RANSOM_EVENTS.map((event, index) => {
        const x = 24 + ((event.start - 142) / 480) * 1128;
        const reached = frame >= event.start;
        return (
          <g key={event.id}>
            <circle
              cx={x}
              cy="78"
              r={reached ? 3.6 : 2.3}
              fill={reached ? (frame >= event.start + 109 ? RC.teal : RC.red) : RC.line2}
            />
            <text
              x={x}
              y="92"
              textAnchor="middle"
              fill={reached ? RC.soft : RC.dim}
              fontSize="6.5"
              letterSpacing="0.8"
            >
              0{index + 1}
...
export const Motion: React.FC = () => {
  ...
  if (frame === 0 || frame >= 899) {
    return <AbsoluteFill style={{backgroundColor: RC.bg}} />;
  }

  const opacity = rcFade(frame);
  const leftReveal = ease(frame, 66, 56);
  const rightReveal = ease(frame, 86, 56);
  const horizonReveal = ease(frame, 105, 54);
  const cameraPeak = interpolate(frame, [470, 610, 730], [1, 1.018, 1.018], {
    ...clamp,
    easing: Easing.inOut(Easing.cubic),
  });
  const peakPulse = frame >= 545 && frame < 716;
  const activeCount = RANSOM_EVENTS.filter(
    (event) => frame >= event.start && frame < event.start + 109,
  ).length;
  const isolated = RANSOM_EVENTS.filter((event) => frame >= event.start + 109).length;
  ...
  return (
    ...
    >
      <svg width="1920" height="1080" viewBox="0 0 1920 1080" style={{position: "absolute"}}>
        <defs>
          <radialGradient id="rc-bg" cx="50%" cy="52%" r="68%">
            <stop offset="0%" stopColor={peakPulse ? "#211016" : "#11191d"} />
            <stop offset="48%" stopColor="#090d11" />
            <stop offset="100%" stopColor={RC.bg} />
          </radialGradient>
          <radialGradient id="rc-disc" cx="50%" cy="48%" r="62%">
            <stop offset="0%" stopColor="#17262a" stopOpacity="0.64" />
            <stop offset="62%" stopColor="#0b1218" stopOpacity="0.8" />
            <stop offset="100%" stopColor="#05080b" stopOpacity="0.25" />
          </radialGradient>
          <linearGradient id="rc-land" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0%" stopColor="#24464b" />
            <stop offset="48%" stopColor="#153236" />
            <stop offset="100%" stopColor="#0d2024" />
          </linearGradient>
          <linearGradient id="rc-title" x1="0" y1="0" x2="1" y2="0">
            <stop offset="0%" stopColor={RC.teal} stopOpacity="0" />
            <stop offset="18%" stopColor={RC.teal} stopOpacity="0.8" />
            <stop offset="82%" stopColor={RC.red} stopOpacity="0.76" />
            <stop offset="100%" stopColor={RC.red} stopOpacity="0" />
          </linearGradient>
          <pattern id="rc-dot-grid" width="42" height="42" patternUnits="userSpaceOnUse">
            <circle cx="2" cy="2" r="1" fill={RC.line2} opacity="0.32" />
          </pattern>
          <filter id="rc-small-glow" x="-200%" y="-200%" width="500%" height="500%">
          ...
          <filter id="rc-heat-glow" x="-300%" y="-300%" width="700%" height="700%">
            <feGaussianBlur stdDeviation="7" />
          </filter>
          <clipPath id="rc-disc-clip">
            <ellipse cx="960" cy="548" rx="607" ry="376" />
          </clipPath>
        </defs>

        <rect width="1920" height="1080" fill="url(#rc-bg)" />
        <rect x="32" y="28" width="1856" height="1024" rx="28" fill="url(#rc-dot-grid)" />
        {Array.from({length: 28}, (_, index) => {
          const x = 36 + seed(index + 180) * 1848;
          const y = 42 + seed(index + 293) * 990;
          const drift = Math.sin(frame * 0.009 + index) * 2.6;
          return (
            <circle
              key={index}
              cx={x}
              cy={y + drift}
              r={index % 5 === 0 ? 1.8 : 0.9}
              fill={index % 7 === 0 ? RC.red : RC.teal}
              opacity={0.08 + 0.2 * (0.5 + 0.5 * Math.sin(frame * 0.022 + index))}
            />
          );
        })}

        <g opacity={opacity} style={{transformOrigin: "960px 560px", transform: `scale(${cameraPeak})`}}>
          <g opacity={ease(frame, 12, 60)}>
            <path
              d="M356 71H632L652 51H1268L1288 71H1564"
              fill="none"
              stroke="url(#rc-title)"
              strokeWidth="2"
            />
            <rect x="739" y="48" width="442" height="58" rx="29" fill={RC.glass} stroke={RC.line2} />
            <circle
              cx="778"
              cy="77"
              r={5.5 + 1.4 * (0.5 + 0.5 * Math.sin(frame * 0.075))}
              fill={peakPulse ? RC.red : RC.teal}
            />
            <text x="803" y="73" fill={RC.ink} fontSize="15" fontWeight="700" letterSpacing="3.1">
              GLOBAL RANSOMWARE
            </text>
            <text x="803" y="91" fill={RC.dim} fontSize="7.5" letterSpacing="1.8">
              CONTAINMENT NETWORK / LIVE TELEMETRY
            </text>
            <text x="1146" y="82" textAnchor="end" fill={peakPulse ? RC.red : RC.teal} fontSize="9" fontWeight="700">
              {peakPulse ? "THREAT PEAK" : "ACTIVE"}
            </text>
            <text x="66" y="87" fill={RC.dim} fontSize="8" letterSpacing="1.5">
              GRID 04 / UTC 18:42:{String(11 + Math.floor(frame / 60) % 49).padStart(2, "0")}
            </text>
            <text x="1854" y="87" textAnchor="end" fill={RC.dim} fontSize="8" letterSpacing="1.5">
              DEFENSE FABRIC / ZERO TRUST
            </text>
          </g>

          <g opacity={ease(frame, 30, 66)}>
            <ellipse cx="960" cy="548" rx="607" ry="376" fill="url(#rc-disc)" stroke={RC.line2} />
            <ellipse
              cx="960"
              cy="548"
              rx="575"
              ry="350"
              fill="none"
              stroke={peakPulse ? RC.red : RC.teal}
              strokeOpacity={peakPulse ? 0.34 : 0.22}
            ...
            {[0, 1, 2].map((ring) => (
              <ArcMeter
                ...
                frame={frame}
                phase={ring * 91}
                color={ring === 2 && peakPulse ? RC.red : ring === 1 ? RC.amber : RC.teal}
                reverse={ring === 1}
              />
            ))}
            <g clipPath="url(#rc-disc-clip)">
              ...
              <ContainmentMap frame={frame} />
            </g>
            <RcCorners x={392} y={208} width={1136} height={676} color={peakPulse ? RC.red : RC.line2} opacity={0.72} />
          </g>

          <ThreatRail frame={frame} reveal={leftReveal} />
          <ResponseRail frame={frame} reveal={rightReveal} />
          <EventHorizon frame={frame} reveal={horizonReveal} />

          <g opacity={ease(frame, 110, 50)}>
            <g transform="translate(420 845)">
              <rect width="222" height="50" rx="14" fill={RC.glass} stroke={RC.line} />
              <text x="16" y="20" fill={RC.dim} fontSize="7.5" letterSpacing="1.1">
                ACTIVE INFECTIONS
              </text>
              <text x="16" y="39" fill={activeCount > 0 ? RC.red : RC.teal} fontSize="16" fontWeight="700">
                {String(activeCount).padStart(2, "0")}
              </text>
              <circle cx="199" cy="25" r={4 + (activeCount > 0 ? 2 * (0.5 + 0.5 * Math.sin(frame * 0.1)) : 0)} fill={activeCount > 0 ? RC.red : RC.teal} />
            </g>
            <g transform="translate(1278 845)">
              <rect width="222" height="50" rx="14" fill={RC.glass} stroke={RC.line} />
              <text x="16" y="20" fill={RC.dim} fontSize="7.5" letterSpacing="1.1">
                REGIONS ISOLATED
              </text>
              <text x="16" y="39" fill={RC.teal} fontSize="16" fontWeight="700">
                {String(isolated).padStart(2, "0")} / 06
              </text>
              <path
                d="M182 28L191 36L207 17"
                fill="none"
                stroke={RC.teal}
                strokeWidth="2.2"
                strokeLinecap="round"
                strokeLinejoin="round"
                opacity={isolated > 0 ? 1 : 0.25}
              />
            </g>
          </g>
        </g>
